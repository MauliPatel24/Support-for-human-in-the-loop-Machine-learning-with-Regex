var Regex101Colorizer = function() {
    ...
    function t(t, o, p, i, s) {
        var a, l, c, _ = g.exec(t),
            y = [],
            h = {
                rangeable: !1,
                type: regex_type.NONE
            },
            _ = {
                opening: _[1],
                content: _[2],
        ...
        if (1 === e("posix", N) && /^\[:.*?:\]$/.test(t)) a = {
            ...
            for (a = {
                ...
                if (c = l[0], "\\" === c.charAt(0))
                    if (/^\\[cuxE]$/.test(c) && 1 === e(c.charAt(1), N)) y.push({
                        contents: c,
                        error: !0,
                    ...
                    y.push({
                        contents: c.substring(0, 2),
                        type: regex_type.QUOTE
                    });
                    var f = c.substring(c.length - 2, c.length),
                        x = c.length;
                    i += "\\Q", "\\E" === f && (x -= 2);
    ...
    function o(r, n, o) {
            ...
            for (var p = t, i = 0, l = e.length; l > i; i++) {
                ...
                if (!c.error) {
                    if (c.type === regex_type.TEXT) {
                        var _ = c.contents; - 1 !== v.modifiers.indexOf("x") && (_ = _.replace(/\s+/g, "")), p += _.length
                    } else if (c.type === regex_type.ESCAPED_TEXT) p += c.contents.length - 1;
                    else if (c.type === regex_type.CHARCLASS) p++;
                    else {
        ...
        var c, _, u, g, x, A = [{
            ...
            w = "";
        O = [], R = 0, I = !1, P = -1, m = "", b = n;
        for (var K = !1; c = y.exec(r);)
            if (_ = c[0], u = _.charAt(0), g = _.charAt(1), K) {
                if ("\n" === _ || "\r" === _) K = !1, v.group_contents += _;
                else if (/\n/.test(_)) {
                    K = !1;
                    var $ = _.split("\n");
            ...
            var Q = {
                contents: _,
                type: regex_type.COMMENT,
                quantifiable: !1
            };
            l(v, Q), v.children.push(Q), K = !0
        } else {
            if ("[" === u) {
                var q = o === c.index || o + 1 === c.index + _.length && "]" === r.charAt(c.index + _.length - 1) || "^" === _.charAt(1) && o === c.index + 1,
                    V = t(_, q, v.modifiers, w, G);
                ...
                if (x = /^\(\*[\w:]*\)?/.exec(_)) {
                    ...
                    var q = o >= c.index && o < c.index + _.length,
                        W = {
                            unmatched: !0,
                    ...
                    if (l(v, W), "(" === _) W.type = regex_type.CAPTURING_GROUP, R++, W.ref_num = R, v.type === regex_type.SUBPATTERN_GROUP && (W.subpatterns = v.subpatterns, W.currSubpatterns = v.currSubpatterns, W.currSubpatterns.push(""), W.subpatterns.push(""));
                    else if (_.length > 3 && /^\(\?[imsxXUJ-]*:$/.test(_)) W.modifiers = p(i(v.modifiers, _.substring(2, _.length - 1))), S = -1 !== W.modifiers.indexOf("x"), F = -1 !== W.modifiers.indexOf("U"), D = -1 !== W.modifiers.indexOf("J"), W.type = regex_type.GROUP_MODIFIER;
                    else if ("(?|" === _) W.type = regex_type.SUBPATTERN_GROUP, W.capturingGroupCount = R, W.subpatterns = [], W.currSubpatterns = [];
                    else if (/\(\?(?:P?<\w+>|'\w+')$/.test(_)) {
                ...
                if (0 === v.level) v.group_contents += _, v.children.push({
                    contents: ")",
                    error: !0,
                    ...
                    var te = A.pop();
                    if (te.unmatched = !1, te.group_contents += ")", te.selected || (te.selected = o == c.index), v = A[A.length - 1], v.group_contents += te.group_contents, v.type === regex_type.CONDITIONAL)
                        if (1 === v.children.length)
                            if (d.test(te.group_contents)) {
                    ...
                    if (te.selected && te.alternations > 0)
                        for (var ne = 0, oe = te.children.length; oe > ne; ne++) {
                ...
                if (/^\d/.test(g)) {
                    ...
                    if (-1 !== "uLlUE".indexOf(g) && -1 === e(g, N)) ce.error = !0, ce.error_type = error_type.UNSUPPORTED_TOKEN;
                    else if (/^\\[cxkguE]$/.test(_)) ce.error = !0, ce.error_type = error_type.INCOMPLETE_TOKEN;
                    else if ("GAZzBbK".indexOf(g) > -1) ce.quantifiable = !1;
                    else if ("x" === g) {
                    ...
                    } else if ("pP".indexOf(g) > -1) ce.error = !E.test(_), ce.error_type = 2 === _.length || "{" === _.charAt(2) && "}" !== _.slice(-1) ? error_type.INCOMPLETE_TOKEN : error_type.UNKNOWN_SCRIPT;
                    else if ("Q" === g) {
                        ce.type = regex_type.QUOTE, ce.contents = "\\Q", w += "\\Q", v.children.push(ce);
                        var ye, ue = _.substring(2),
                            ge = _.substring(_.length - 2, _.length);
                        if ("\\E" === ge) {
                            ...
                            for (var he = ue.split(""), de = 0, fe = he.length; fe > de; de++) {
                                var Ee = {
                                    contents: he[de],
                                    type: regex_type.QUOTE_TEXT,
                                    quantifiable: !1
                ...
                if (H)
                    if (H.type !== regex_type.QUANTIFIER && H.type !== regex_type.GROUP_QUANTIFIER || void 0 === H.prev || void 0 === H.prev.quantifier)
                        if (H.error) Te.error = !0, Te.error_type = error_type.NOT_QUANTIFIABLE;
                        else if (H.quantifiable) {
                    var Ne = /^{([0-9]+)(,([0-9]*))?/.exec(_);
                    if (Ne && (H.from = Ne[1], H.to = Ne[2] ? Ne[3] : Ne[1]), Ne && (+Ne[1] > 65535 || Ne[3] && +Ne[3] > 65535)) Te.error = !0, Te.error_type = error_type.TOO_LARGE_QUANTIFIER;
                    else if (Ne && Ne[3] && +Ne[1] > +Ne[3]) Te.error = !0, Te.error_type = error_type.BAD_QUANTIFIER_RANGE;
                    else if (v.lookbehind && !/^{(\d+)(?:,\1)?}$/.test(_)) Te.error = !0, Te.error_type = error_type.LOOKBEHIND_QUANTIFIER;
                    else {
                        if (isGroupType(H.type) && (Te.type = regex_type.GROUP_QUANTIFIER, Te.level = H.level), (isGroupType(H.type) || H.type === regex_type.CHARCLASS) && (Te.selected = H.selected || o >= c.index && o < c.index + _.length, Te.selected && (H.selected = !0, H.alternations > 0)))
                            for (var ne = 0, Oe = H.children.length; Oe > ne; ne++) {
                                var pe = H.children[ne];
                                pe.type === regex_type.ALTERNATOR && (pe.selected = !0)
                            }
                        H.quantifier = _, H.quantifier_token = Te, H !== v && H.group_contents && (H.group_contents += _)
                    }
                } else Te.error = !0, Te.error_type = error_type.NOT_QUANTIFIABLE;
                else H.type === regex_type.GROUP_QUANTIFIER && (Te.type = regex_type.GROUP_QUANTIFIER, Te.level = H.level, H.prev.group_contents += _), Te.error = H.error || !h.test(H.contents + _), Te.error_type = H.error_type || error_type.NOT_QUANTIFIABLE, Te.selected = H.selected, H.prev.quantifier += _, H.Umode = F;
                else Te.error = !0, Te.error_type = error_type.NOT_QUANTIFIABLE;
        ...
        for (var ne = 0, be = M.length; be > ne; ne++) {
            ...
            if (!ve.error && "(R)" !== ve.group_contents && "(?R)" !== ve.contents && "(?0)" !== ve.contents) {
                var Z = ve.contents.replace(/^(?:\\[gk]|\((?:R|\?P[=>]))|[^-+\w]/g, ""),
                    ie = parseInt(Z);
    ...
    function c(e, r, t, n, o, p) {
        ...
        for (var _ = 0, y = e.length; y > _; _++) {
            var u = e[_],
                g = u.contents;
            if (n || t.injected) g = white_space(escapeHtml(g));
            else if (t.currentPos += u.contents.length, t.currentPos > t.pos) {
    ...
    return A.colorize = function(e, r, t, n, o, p, i) {
        ...
        var o = {
            currentPos: 0,
            injected: t,
            pos: getCaretPosition(document.getElementById("regex"))
        };
